...
class NSDepartureSensor(SensorEntity):
    ...
    def extra_state_attributes(self):
        ...
        if self._trips[0].departure_time_planned is not None:
            ...
            if planned_time > datetime.now().time():
                attributes["departure_time_planned"] = planned_time.strftime("%H:%M")
        else:
            # If the planned departure time is None or in the past, look for the next trip with a valid planned departure time
            for trip in self._trips[1:]:
                if trip.departure_time_planned is not None and trip.departure_time_planned > datetime.now().time():
        ...
        if self._trips[0].departure_time_actual is not None:
            ...
            if actual_time > datetime.now().time():
                attributes["departure_time_actual"] = actual_time.strftime("%H:%M")
        else:
         # If the actual departure time is None or in the past, look for the next trip with a valid actual departure time
            for trip in self._trips[1:]:
                if trip.departure_time_actual is not None and trip.departure_time_actual > datetime.now().time():
